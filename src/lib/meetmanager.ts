    ...
    for (const eventId of swimmer.selectedEvents) {
      ...
      if (targetMeet.availableEvents.includes(eventId)) {
        ...
        if (event && !event.isRelay) {
          ...
          // D0 record: positions 1-161 (extended record)
          // D0 + Org + future use + swimmer name + USS# + attach + citizen + birth date + age/class + sex + event sex + distance + stroke + future use + event age + date of swim + seed time + course + prelim time + course + swim-off time + course + finals time + course + prelim heat + prelim lane + finals heat + finals lane + prelim place + finals place + points + time class + flight status + future use
          ...
          const seedTimeD0 = seedTime; // positions 89-96
          ...
          content += `D0001${futureUseD0_1}${swimmerName}${ussNumber}${attachCode}${citizenCode}${swimmerBirthDate}${swimmerAgeClass}${sexCode}${eventSexCode}${eventDistance}${strokeCode}${futureUseD0_2}${eventAgeCode}${dateOfSwim}${seedTimeD0}${courseCodeD0_1}${prelimTime}${courseCodeD0_2}${swimOffTime}${courseCodeD0_3}${finalsTime}${courseCodeD0_4}${prelimHeat}${prelimLane}${finalsHeat}${finalsLane}${prelimPlace}${finalsPlace}${pointsScored}${eventTimeClass}${flightStatus}${futureUseD0_3}\n`;
        }
      }
    }
  }
