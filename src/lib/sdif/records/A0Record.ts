...
export class A0Record {
  static generate(
    ...
    const futureUse1 = (fileDescription || '').padEnd(30, ' '); // positions 14-43, future use
    ...
    const contactNameField = (contactName || 'Team Manager').padEnd(20, ' ').substring(0, 20); // positions 74-93
    const contactPhoneField = (contactPhone || '').padEnd(12, ' ').substring(0, 12); // positions 94-105
    ...
    return `A0${orgCode}${sdifVersion}${fileCode}${futureUse1}${softwareName}${softwareVersion}${contactNameField}${contactPhoneField}${fileCreationDate}${futureUse2}${lscCode}${futureUse3}\n`;
  }
}
